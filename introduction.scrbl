...
@align[r.l.n @list[
 @f{S_{\rm BV}\;=\;}
 @f{S_{\rm mat}[I(b^{\star})\,,\, x] \;+\; \int\;\langle b,{\cal L}_c(I(b^{\star}))\rangle + \langle x^{\star}, {\cal L}_c x\rangle + {1\over 2} \langle c^{\star} [c,c]\rangle}
 @label{BVBosonic}
]
]
...
@f{  T_{\alpha\beta}{\partial\over\partial b_{\alpha\beta}} 
   ...
   + {1\over 2} [c,c]^{\alpha}{\partial\over\partial c^{\alpha}}
}
""
]]
...
@align[r.l.n @list[
  @f{\Delta_{\rho_{1/2}}F\;=\;}
  @f{{1\over\rho_{1/2}}\Delta(F\rho_{1/2})=\Delta^{(0)}F + \{F,S_{\rm BV}\}}
  ""
]
]
But we have a problem: @f{\Delta^{(0)}} @bold{is not well-defined on local expressions}. 

We can define it ``by hand'', just to be zero on local expressions:

@div[redbox]{Local functionals form a Lie superalgebra under @f{\{\_,\_\}}; we define
@f{\Delta^{(0)}} as the differential of its homology complex}

But we have to pay the price: 
@div[redbox]{
With this artificial definitions of @f{\Delta}, the theorems about the variation of @f{\int_L}
generally speaking fail. Such theorems are needed to prove the closedness of @f{\Omega}.
}
However, some things we can still count on. First of all:
@itemlist[
@item{@f{\int_L\exp\left({1\over\hbar}S_{\rm BV}\right)} still does not change under small deformations of @f{L}}
]
This is because we restrict ourselves to @bold{non-anomalous theories}. The nontrivial
dependence of @f{\int_L\exp\left({1\over\hbar}S_{\rm BV}\right)} on the choice of @f{L} is the
...
This would seem to be potentially dangerous, as we are using the ``regularized'' @f{\Delta^{(0)}} in the
formula for Lie derivative in Eq. (@ref{integral-of-regularized-Lie-derivative}). But the key point
is, that one can always (at least locally) choose such Darboux coordinates that @f{\Psi} only
depend on fields and not on antifields. With such coordinates, the effect of regularization
in our ``homological'' definition of  @f{\Delta^{(0)}} does not matter. 
@comment{
Anomalies appear as effect of Jacobian of a field redefinition. When  @f{\Psi} is chosen to only
depend on fields, the manipulations needed to prove the closedness of @f{\Omega} do not actually
involve field redefinitions. Therefore there is no anomaly in the closedness of @f{\Omega}.
}
However, while there are no anomalies in the closedness of  @f{\Omega}, there are
potentially anomalies in the closedness of  its base analogue @f{\Omega^{\tt base}}.
Indeed, in the proof of closedness of @f{\Omega^{\tt base}}, we need to act with @f{\Delta^{(0)}}
on @f{e^a}. In bosonic string theory, as in any theory coming from BRST formalism, it is true that @f{a}  
only depends on antifields — see Eq. (@ref{a-in-BRST-formalism}) where @f{\Phi = a}.
Therefore, the effect of regularization should not cause anomalies. However, in the
pure spinor string @f{a} involves both fields and antifields. Therefore:
@div[redbox]{For the pure spinor string we need to also prove the non-anomalousness of the 
symmetry generated by the @f{b}-ghost. This is needed for the construction of the base form @f{\Omega^{\tt base}}.
}
