...
the left shift @f{\widehat{g}\mapsto \widehat{h}\widehat{g}}. However,
...
where @f{H} is the Lie group generating by the flows of elements of @f{\bf h}.
}


@slide[@elem{Subspace @f{{\cal F}\subset \mbox{Fun}(M)}} #:tag "Subalgebra F" #:showtitle #t]{

The construction starts with a choice of a subspace @f{{\cal F}\subset \mbox{Fun}(M)} which must satisfy
some special properties which we will now describe. First of all, we require the existence of a map
@equation{
...
which satisfies @f{\Delta [x,y] = \{\Delta x,\Delta y\}}. We will impose the following additional
requirement:
...
We will now build an @f{\bf h}-equivariant analogue of @f{\Omega}.
}
...
@centered{
@spn[redbox]{on this page we will restrict ourselves to @bold{even} @f{\Phi\langle\xi\rangle},
@italic{i.e.} odd @f{\xi}
}}
Let us define a function @f{{\bf h}\to \mbox{Fun}(M)} as follows:
@align[r.l.n @list[
   @f{\xi\;\mapsto\;} @f{\Gamma(\Phi\langle \xi\rangle) = 
1 + \Phi\langle\xi\rangle + \Gamma_2(\Phi\langle\xi\rangle ,\Phi\langle\xi\rangle) + 
\Gamma_3(\Phi\langle\xi\rangle, \Phi\langle\xi\rangle, \Phi\langle\xi\rangle) + \ldots} ""
]
]
where @f{\Gamma_n(\Phi,\ldots,\Phi)} is a symmetric polilinear function, as follows:
@align[r.l.n @list[
@f{\Gamma =\;}@f{e^a}@label{GammaIsEa}
]]
where @f{a} is defined as the perturbative solution of the equation:
@equation[#:label "EquationForLogForEquivariant"]{
\Delta a + {1\over 2} \{a,a\} = \Delta\Phi
}
@centered{@spn[redbox]{we abbreviate @f{\Phi\langle\xi\rangle} to just @f{\Phi}}}
The ``perturbative solution'' is defined as a series in @f{\Phi}:
@equation{
a = \Phi - {1\over 2} q(\Phi,\Phi) + \ldots
}
The @bold{consistency} of this equation requires that  @f{\Delta \{a,a\} =0} at
...
Notice that @f{\Gamma[\Phi]} satisfies:
@equation[#:label "DefGamma"]{
   \Delta \Gamma(\Phi) = (\Delta \Phi)\Gamma(\Phi)
}
With this @f{\Gamma(\Phi)}
@align[r.l.n @list[
@f{\Omega_{\bf h}^{\tt C}\;\in \;}
@f{S^{\bullet}({\bf h}^*) \otimes \Omega^{\bullet}({\rm LAG})}
""
]@list[
@f{\Omega_{\bf h}^{\tt C} (\xi) =\;}
@f{\Omega\langle \Gamma(\Phi\langle\xi\rangle)\rangle}
@label{OmegaCartanCalF}
]]
is a cocycle of the Cartan model of the @f{\bf h}-equivariant cohomology of @f{G}. In other words:
@align[r.l.n @list[
  @f{\Omega_{\bf h}^{\tt C} (\Phi) \;=\;}
