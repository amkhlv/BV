...
@(define bystro-conf   
   (bystro (bystro-connect-to-server (build-path (find-system-path 'home-dir) ".config" "amkhlv" "latex2svg.xml"))
           "unintegrated-vertex/formulas.sqlite"  ; name for the database
           "unintegrated-vertex" ; directory where to store image files of formulas
           25  ; formula size
...
@section{Fixing the Lagrangian submanifold}
...
@subsection[#:tag "NotationsAndUsefulIdentities"]{Notations and useful identities}
@align[r.l.n @list[
...
@subsection{The two-form component}
@align[r.l.n @list[
 "" 
...
@section{Definition of the Lie algebra cohomology @f{H^{\bullet}({\bf h}, \mbox{Fun}(M))}}
...
@section[#:tag "sectionCohomologyOfDiffeomorphisms"]{Integrated vertex and Lie algebra cohomology}
...
@section{Comparison of @f{e^{\{\Psi,\_\}}} and @f{e^{\Delta\Psi}}}
...
@section[#:tag "sectionIndegratedVsUnintegrated"]{Relation between integrated and unintegrated vertices}
...
@section{Various types of Lie algebra cohomology}
...
@section{Partially quantum Master Equation}
